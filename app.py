...
# ==== ПЕРЕМЕННЫЕ (Ellen Clinic) ====
BOT_TOKEN  = os.environ.get('BOT_TOKEN', '8365324980:AAHRskXrI0NOPttX6r2xDkLhfYWcOtq0YKU')
SUBSCRIPTION_CHANNEL_ID = os.environ.get('SUBSCRIPTION_CHANNEL_ID', '-1002188248652')
LEADS_TARGET_ID         = os.environ.get('LEADS_TARGET_ID', '-1003167450331')

# ВАЖНО: BOT_USERNAME нужен БЕЗ @ и без "t.me/"
BOT_USERNAME = os.environ.get('BOT_USERNAME', 'Ellen_Clinic_Tula_bot').strip('@')

# Админы (главный + доп.админы для админки)
_admin_env = os.environ.get(
    'ADMIN_USER_IDS',
    '291655689,1826123313,554278522'
).replace(' ', '')
...
# REFERRAL_BONUS — +2 попытки за каждого приглашённого по рефералке (навсегда)
...
# ==== БАЗОВЫЕ ПРИЗЫ Ellen Clinic (фолбэк, если нет конфигурации в БД) ====
# label (короткий) + win_text (полный текст, который увидит победитель)
PRIZES = [
...
def get_wheel_config():
    ...
    if conf:
        ...
        items = []
        for i, (l, w) in enumerate(zip(PRIZES, PRIZE_WEIGHTS)):
            wt = PRIZES_WIN_TEXT[i] if i < len(PRIZES_WIN_TEXT) else ''
            items.append({'label': l, 'weight': w, 'win_text': wt})
